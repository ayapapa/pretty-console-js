...
| `onLog`       | `(logEntry: LogEntry) => void` or `null` | `null` | A callback function that receives each log call before level filtering and formatting. The callback may modify LogEntry.args, and any changes are reflected in subsequent PrettyConsole processing. |
